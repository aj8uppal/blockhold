...
import {studyModel,directions,forms,type Direction} from './models.ts'
import {FireStudy,fireDirections,type FireDirection} from './fire.ts'
...
let selected=params.get('direction')??options[0].id,index=Math.min(8,Math.max(0,Number(params.get('form')??5))),view=params.get('view')??'studio',pairModels:THREE.Group[]=[]
...
function show(direction=selected,form=index){
  selected=options.some(d=>d.id===direction)||direction==='current'?direction:options[0].id;index=Math.min(8,Math.max(0,Math.floor(form)))
  ...
  if(mode==='seraph'){
    const f=forms[index],id=`seraph${f.tier}${f.tier>=4?f.branch===0?'a':'b':''}` as TowerModelId
...
function animatePose(t:number){
  if(model){
    const heart=model.getObjectByName('heart');if(heart)heart.scale.setScalar(1+Math.sin(t*1.8)*.018)
    // Carved stone and buildings are grounded. Only the suspended relic breathes.
    if(selected==='relic'&&heart)heart.position.y=(forms[index].tier<4?6+forms[index].tier:10.1+Math.max(0,forms[index].tier-4)*.65)*.1+Math.sin(t*1.35)*.025
  }
  fire?.update(t)
  ...
  if(active&&model){
    ...
    beamMat.color.set(dark?0x9972d6:0xffe4a0)
...
;(window as any).artStudy={show,pair,info,setView:(v:string)=>{view=v;framing()},freeze:(t=1.7)=>{motion(false);time=t;animatePose(t);engine.render(true)},play:()=>motion(true),attack:()=>{attackAt=time;motion(true)},capture:(on=true)=>document.body.classList.toggle('capture',on),engine}
