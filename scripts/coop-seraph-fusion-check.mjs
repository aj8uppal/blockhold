/** Two real browser seats: competing Seraph sacrifices, combat, rejoin and solo recovery.
...
try {
  ...
  await host.evaluate(() => {
    ...
    const plots=[plot,...g.terrain.plots.filter(p=>p!==plot&&!p.water)]
    for(const branch of[0,1]){g.buildTower('seraph',plots[branch]);for(let i=0;i<5;i++)g.upgradeTower(g.towers[branch],i===2?branch:0)}
  })
  ...
  await Promise.all(pages.map(p=>p.waitForFunction(()=>window.vg.game.towers.length===1&&window.vg.game.towers[0].isFused)))
  ...
  console.log('PASS: competing sacrifices consume one donor; Crimson combat, rejoin and solo recovery match state hashes across both real browsers.')
