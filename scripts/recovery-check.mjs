...
const withMastery = process.env.BLOCKHOLD_CHECK_MASTERY === '1'
...
try {
  for (let i = 0; i < 2; i++) {
    ...
    await context.addInitScript(({ battle, player, withMastery }) => {
      if (!localStorage.getItem('blockhold.recovery-check')) {
        localStorage.setItem('blockhold.save.v1', JSON.stringify({ xp: player ? 7000 : withMastery ? 20000 : 5000,
          honors: !player && withMastery ? ['mastery:seraph:ossuary', 'mastery:seraph:empress'] : [],
          taughtBasics: true, sfxMuted: true, musicMuted: true }))
  ...
  if (withMastery) assert.equal(await host.evaluate(() => window.vg.game.mythicLock(window.vg.game.towers[0])), null)
  ...
  if (withMastery) {
    assert.equal(await guest.evaluate(() => window.vg.game.mythicLock(window.vg.game.towers[0])), null)
    assert.deepEqual(await guest.evaluate(() => window.vg.game.save.honors ?? []), [])
  }
