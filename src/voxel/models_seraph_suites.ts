...
// Shared voxel art directions. The selected game models use Crystal Wings and Sacred Stone.
...
export function sacredStone(tier: number, branch: number): VoxModel {
...
export function crystalWings(tier:number,branch:number):VoxModel{
  const dark=tier>=4&&branch===1,solar=tier>=4&&!dark
  const rank=Math.max(0,tier-4)
  const core=dark?0x392d55:solar?0xe7b552:0xa7b6df
  const face=dark?0x76618f:solar?0xffe3a1:0xe0e8fa
  ...
  if(tier===1){gem(parts.heart,0,cy,1.25,.9,3.1,0xc2d8ff,0xf0f4ff,true)}
    ...
    for(const side of [-1,1]){
      ...
      const span=tier===2?4.2:tier===3?5.1:5.8+rank*.55
      const elbowY=cy+(dark?-1.0:2.4),tipY=cy+(dark?-4.8-rank*.2:4.5+rank*.25)
    ...
    if(dark){disk(parts.heart,cy,1.6,1.1+rank*.12,0x100d1b);ring(parts.heart,0,cy,1.95,1.2+rank*.12,0x9865dc,.12,true)}
    // A bird crest for Solar and an upward lunar fork for Void.
    const crownY=cy+3.1+rank*.2
    gem(parts.figure,0,crownY,.1,.75+rank*.05,2.9+rank*.2,core,face)
    if(tier>=3)for(const side of [-1,1]){
      stroke(parts.figure,side*.4,crownY,side*(1.3+rank*.15),crownY+2+rank*.3,.1,.55,.5,edge)
      if(solar)stroke(parts.figure,side*.2,crownY+.1,side*(1.8+rank*.2),crownY+.5,.1,.5,.5,edge)
    }
