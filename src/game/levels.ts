...
export function generateEndlessWaves(level: LevelDef, count = 999): WaveDef[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const pool = [
      ...
      ['shield', 'gargoyle', 'acolyte', 'warlock', 'mistwalker', 'spiderling', 'shardback'],
      ['shield', 'gargoyle', 'acolyte', 'warlock', 'mistwalker', 'broodmother', 'brute', 'shardback'],
    ][tier]
    const groups = []
    const groupCount = 2 + Math.min(3, Math.floor(i / 6))
    for (let gi = 0; gi < groupCount; gi++) {
      const enemy = pick(pool)
      const base = enemy === 'spiderling' ? 8 : enemy === 'brute' || enemy === 'broodmother' ? 1 : 3
      const cnt = Math.max(1, Math.round(base * (1 + i * 0.045) * (0.7 + rand() * 0.6)))
      groups.push({
        enemy,
        count: enemy === 'brute' || enemy === 'broodmother' ? Math.min(cnt, 3 + Math.floor(i / 12)) : cnt,
    ...
    // bosses stride in every tenth wave, alternating
    if (i > 0 && i % 10 === 9) {
      groups.push({ enemy: i % 20 === 19 ? 'veilqueen' : 'juggernaut', count: 1, interval: 1, delay: 8, lane: lane() })
    }
...
export const EARLY_CALL_GOLD_PER_SEC = 1.4
