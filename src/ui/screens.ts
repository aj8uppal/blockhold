...
const THEME_EMOJI: Record<string, string> = { forest: '🌲', winter: '❄️', ember: '🌋', swamp: '🍄', void: '👑' }
...
export class Screens {
  ...
  private renderMenu(): void {
    ...
    el('div', 'menu-crest', card, '🏰')
    el('h1', 'game-title', card, 'BLOCKHOLD')
    el('div', 'game-tagline', card, 'Hold the line, block by block.')
    const play = el('button', 'btn primary big', card, '⚔️ &nbsp;To Battle') as HTMLButtonElement
  ...
  private renderLevels(): void {
    ...
    const armoryBtn = el('button', 'btn ghost small', head, `⚔️ Armory · ${starsAvailable(save)}★`) as HTMLButtonElement
    ...
    levels.forEach((lvl, i) => {
      ...
      const art = el('div', 'level-art', card, locked ? '🔒' : THEME_EMOJI[lvl.theme])
      ...
      el('div', 'level-stars', card, '★'.repeat(stars) + '<span class="dim">' + '★'.repeat(3 - stars) + '</span>' +
        (medals.includes('noleak') ? '<span class="level-medal" title="Flawless: won without a single leak"> 🏅</span>' : '') +
        (medals.includes('veteran') ? '<span class="level-medal" title="Conquered on Veteran"> 🎖️</span>' : '') +
        (best > 0 ? `<span class="level-endless"> 🌙${best}</span>` : ''))
  ...
  private showDifficultyPicker(levelId: string, levelName: string): void {
    ...
    if (beaten) {
      ...
      mkMode('campaign', '⚔️ Campaign')
      mkMode('endless', `🌙 The Long Night${best > 0 ? ` · best ${best}` : ''}`)
    }
    ...
    for (const def of Object.values(HERO_DEFS)) {
      ...
      btn.innerHTML = `<span class="hero-icon">${def.icon}</span>` +
        `<span class="hero-name">${def.name}</span><span class="hero-title">${def.title}</span>` +
        `<span class="hero-blurb">${def.blurb}</span>` +
        `<span class="hero-stats">❤️ ${def.hp} · ⚔️ ${def.damage[0]}–${def.damage[1]}${def.attackRange ? ` · 📏 ${def.attackRange}` : ' · melee'}</span>` +
    ...
    for (const key of ['casual', 'normal', 'veteran'] as Difficulty[]) {
      ...
      btn.innerHTML = `<span class="diff-name">${d.name}</span><span class="diff-blurb">${d.blurb}</span>` +
        `<span class="diff-stats">❤️ ${d.lives} · foes ${Math.round(d.enemyHp * 100)}% · gold ${Math.round(d.bounty * 100)}%</span>`
  ...
  private renderEnd(won: boolean, stars: number, levelId: string, stats?: BattleStats): void {
    ...
    el('div', 'end-emoji', card, endless ? '🌙' : won ? '🏆' : '💀')
    ...
    if (endless && stats) {
      el('div', 'end-sub', card,
        ...
        (stats.newWaveRecord ? ' — a new record! 🏅' : ` · best: ${stats.bestEndless}`))
    } else if (won) {
      ...
      el('div', 'end-sub', card, stats?.noleak ? 'A flawless defense — not one got through! 🏅'
    ...
    if (stats) {
      ...
      el('div', 'end-stats', card,
        `⚔️ ${stats.kills} slain · 🛡 ${stats.perfectWaves} waves held${stats.bestStreak >= 2 ? ` (🔥×${stats.bestStreak})` : ''} · ` +
        `🪙 ${stats.gold} · 💎 ${stats.shards} · 🤺 lvl ${stats.heroLevel}${stats.heroKills > 0 ? ` ☠${stats.heroKills}` : ''} · ⏱ ${fmtTime(stats.timeSec)}`)
      if (stats.topKiller && stats.topKiller.kills > 0) {
        el('div', 'end-topkiller', card, `🏆 Deadliest building: <b>${stats.topKiller.name}</b> — ${stats.topKiller.kills} slain`)
      }
    }
  ...
  private renderArmory(): void {
    ...
    const rerender = () => {
      ...
      for (const track of ARMORY_TRACKS) {
        ...
        el('div', 'ai-icon', item, track.icon)
  ...
  private renderHelp(): void {
    ...
    card.insertAdjacentHTML('beforeend', `
      <div class="help-grid">
        ...
        <div><b>☄️ Abilities.</b> Meteor Storm (1) devastates an area. Reinforcements (2) plug a leak for a few seconds.</div>
        <div><b>🗡 Trap the road.</b> Rune circles on the road hold traps: spike snares, frost runes, and blast charges that fire on whatever crosses them.</div>
        <div><b>💎 Harvest shards.</b> Shardbacks, elites, and bosses drop Veilshards. Spend them to Overcharge a tower's attack speed or Ascend a tier-4 tower with a permanent perk.</div>
        <div><b>🌑 Respect the Veiltide.</b> Marked waves surge with empowered enemies under a violet sky. Calling one early is a gamble.</div>
        <div><b>🔗 Build in choirs.</b> Same-family towers standing adjacent resonate: +6% damage each (barracks: tougher soldiers).</div>
        <div><b>👁 Know your enemy.</b> Hover any foe to inspect it. Armor shrugs off arrows; mystics resist magic; flyers sail over soldiers and cannons — and Mistwalkers phase out of reach entirely.</div>
        <div><b>⭐ Spend your stars.</b> Victory stars buy permanent upgrades in the Royal Armory, found on the level-select screen.</div>
        <div><b>🌙 Survive the Long Night.</b> Beat a map to unlock its Endless mode: ever-escalating waves, a boss every tenth, and a personal record to chase.</div>
        <div><b>🎥 Camera.</b> Drag to pan; right-drag, middle-drag, or Shift+drag to orbit and tilt; scroll to zoom. Touch: pinch to zoom, twist to rotate, two-finger drag to tilt.</div>
        <div><b>⌨️ Hotkeys.</b> Space = call wave · F = speed · P = pause · H = hero · Q/E rotate · T/G tilt · C = reset view · Esc = cancel/close.</div>
      </div>
    `)
